    ...
    private async void Pop()
    {
        for (var y = 0; y < Height; y++)
        {
            for (var x = 0; x < Width; x++)
            {
                ...
                for (var h = 0; h < ScrollBoard.Instance.Lenght; h++)
                {
                    if(ScrollBoard.Instance.scoreContainers[h].ItemName == tile.Item.nameItem)
                        ScrollBoard.Instance.scoreContainers[h].ScoreItem += tile.Item.value * connectedTiles.Count;
                }
